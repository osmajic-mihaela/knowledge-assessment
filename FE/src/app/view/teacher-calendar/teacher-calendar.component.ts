...
export class TeacherCalendarComponent implements OnInit {
  ...
  viewButton:string = "MESEČNI PRIKAZ"
  ...
  constructor(private  userService:UserService,private quizService:QuizService,private router:Router,private http:HttpClient) {
    this.viewDate = this.getMonday(new Date());
    this.viewDateEnd = addDays(this.viewDate, 6);
    this.files = [];
    //this.teacherId = userService.getCurrentLoggedUser().id
  }
  ...
  private getQuizzesForTeacher(){
    console.log('Uslo u dobavljanje')
    this.quizService.getQuizzesByTeacher(this.teacherId).pipe(
        map((results: Quiz[]) => {
        ...
        (response: CalendarEvent<{ appointment: ScheduleAppStaff }>[]) => {
          console.log('dobavljeni testovi', response)
  ...
  viewQuiz() {
    console.log(this.selectedEvent.meta?.quiz.id!)
    this.quizService.saveCurrentId(this.selectedEvent.meta?.quiz.id!)
    this.router.navigate(['app-view-quiz'])
  }

  viewStudentsResult() {
    this.quizService.saveCurrentId(this.selectedEvent.meta?.quiz.id!)
    this.router.navigate(['app-view-students-results'])
  }
}
